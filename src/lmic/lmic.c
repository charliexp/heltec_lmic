...
// ================================================================================
//
// BEG: CN470 related stuff
//
enum { NUM_DEFAULT_CHANNELS=3 };
static CONST_TABLE(u4_t, iniChannelFreq)[6] = {
    // Join frequencies and duty cycle limit (0.1%)
    CN470_F1|BAND_MILLI, CN470_F2|BAND_MILLI, CN470_F3|BAND_MILLI,
    // Default operational frequencies
    CN470_F1|BAND_CENTI, CN470_F2|BAND_CENTI, CN470_F3|BAND_CENTI,
};
...
bit_t LMIC_setupChannel (u1_t chidx, u4_t freq, u2_t drmap, s1_t band) {
    if( chidx >= MAX_CHANNELS )
        return 0;
    if( band == -1 ) {
        if( freq >= 470300000 && freq <= 470700000 )
            freq |= BAND_DECI;   // 10% 27dBm
        else if( (freq >= 470900000 && freq <= 471100000) ||
                 (freq >= 471700000 && freq <= 471900000)  )
            freq |= BAND_CENTI;  // 1% 14dBm
        else
            freq |= BAND_MILLI;  // 0.1% 14dBm
    } else {
...
static u4_t convFreq (xref2u1_t ptr) {
    u4_t freq = (os_rlsbf4(ptr-1) >> 8) * 100;
    if( freq < CN470_FREQ_MIN || freq > CN470_FREQ_MAX )
        freq = 0;
    return freq;
}
...
#if LMIC_DEBUG_LEVEL > 1
                lmic_printf("%d: Setup channel, idx=%d, freq=%ld\n", os_getTime(), chidx, (unsigned long)freq);
#endif
            }
        }
    }
